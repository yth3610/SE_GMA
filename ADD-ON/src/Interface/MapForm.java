...
class MyFrame extends JFrame{
   ...
   private JButton btnset,btnstart,btnmove;
   ...
   class MovementComponent extends JComponent implements Finals{
	   	  int mapx=1, mapy=1, robotx=1, roboty=1, direction=1;
	   	  
	   	 public MovementComponent(int x, int y, int rx, int ry, int d) {
	         this.mapx = x;
	         this.mapy = y;
	         this.robotx = rx;
	         this.roboty = ry;
	      ...
	      public void paint(Graphics g) {	          
	    	  ...
	    	  switch(direction) {
	    	  case EAST :	// 로봇이 동쪽을 바라보고 있는 경우
	              g.fillArc(20+widthmap*robotx,
	            		  355-heightmap*roboty,50,50,robotEsa,robotEaa);
	              break;
	    	  case SOUTH:	// 로봇이 남쪽을 바라보고 있는 경우
	              g.fillArc(20+widthmap*robotx,
	            		  355-heightmap*roboty,50,50,robotSsa,robotSaa);
	              break;
	    	  case WEST:	// 로봇이 서쪽을 바라보고 있는 경우
	              g.fillArc(20+widthmap*robotx,
	            		  355-heightmap*roboty,50,50,robotWsa,robotWaa);
	              break;
	    	  case NORTH:	// 로봇이 북쪽을 바라보고 있는 경우
	              g.fillArc(20+widthmap*robotx,
	            		  355-heightmap*roboty,50,50,robotNsa,robotNaa);
	              break;
	    	  }
	      }
   }
	...
	public void actionPerformed(ActionEvent e){
         if(e.getSource()==txmap || e.getSource()==txhazard ||
            ...
            try{
               ...
               MapForm.f.setmapComponent(mapcomponent);
   ...
   private class startListener implements ActionListener{
	   public void actionPerformed(ActionEvent e){
		   if(e.getSource()==btnstart){
			   RobotPosition rp = new RobotPosition(); // 로봇이 탐색을 시작한다.
			   Path pathpath = new Path();
			   pathList = pathpath.getPath();
               movedList.add(getRobot().positionSensor());
		   }
	   }
   }
	   ...
	   public void actionPerformed(ActionEvent e){
		   if(e.getSource()==btnmove){
			   ...
			   MovementComponent movecomponent = new MovementComponent(mapposition.getX(), mapposition.getY(),
					   pathList.get(i).getX(),pathList.get(i).getY(),movedList.get(i).getDirection());
